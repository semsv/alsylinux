...
:setupNT
\alsylin\boot\syslinux.exe -maf -d /alsylin/boot/ %DISK%:
if %ERRORLEVEL% == 0 goto setupDone
goto errorFound

:setup95
\alsylin\boot\syslinux.com -maf -d /alsylin/boot/ %DISK%:
if %ERRORLEVEL% == 0 goto setupDone
goto errorFound
...
color 4F
echo.
echo Directory %DISK%:\alsylin\boot\ seems to be on the same physical disk as your Windows.
